...
public class TestRequestQueueController {
    ...
    private void handleMarkAsPaid() {
        ...
        if (selectedRequest.getPaymentStatus().equals("PAID")) {
            messageLabel.setStyle("-fx-text-fill: red;");
        ...
        try {
            ...
            insertAuditLog("MARK_PAYMENT_PAID",
                    "Marked request ID " + selectedRequest.getRequestId() + " as paid.");
    ...
    private void markRequestAsPaid(int requestId) throws SQLException {
        String sql = "UPDATE test_requests "
                   + "SET payment_status = 'PAID', request_status = 'PAID', updated_at = CURRENT_TIMESTAMP "
                   + "WHERE request_id = ?";
